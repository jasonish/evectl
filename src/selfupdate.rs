...
pub(crate) fn is_update_available() -> Result<bool> {
    ...
    let current_exe = match env::current_exe() {
        ...
        Err(err) => {
            warn!(
                "Failed to determine executable name while checking for updates: {}",
                err
            );
            return Ok(false);
        }
    ...
    let current_hash = match current_checksum(&current_exe) {
        Err(err) => {
            warn!(
                "Failed to calculate checksum of current executable while checking for updates: {}",
                err
            );
            return Ok(true);
        }
    ...
    if response.status().as_u16() != 200 {
        error!(
            "Failed to fetch remote checksum while checking for updates: HTTP status code={}",
            response.status(),
        );
        return Ok(false);
    }
    ...
    let remote_hash = match parse_sha256_hash(&remote_hash_text) {
        ...
        None => {
            error!("Remote checksum response was invalid while checking for updates");
            return Ok(false);
        }
    };

    Ok(current_hash != remote_hash)
}
